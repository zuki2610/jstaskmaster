document.addEventListener("DOMContentLoaded", () => {
  ...
  const backToTasksBtn = document.querySelector('[data-js="back-to-tasks-btn"]');
  ...
  userForm.addEventListener("submit", (e) => {
    ...
    if (editUserId !== null) {
      users = users.map(u => u.id === editUserId ? { ...u, name, email, role } : u);
      editUserId = null;
    } else {
  ...
  function renderUsers() {
    ...
    if (sortVal) {
      switch (sortVal) {
        case "name-asc": users.sort((a,b)=>a.name.localeCompare(b.name)); break;
        case "name-desc": users.sort((a,b)=>b.name.localeCompare(a.name)); break;
        case "role": users.sort((a,b)=>a.role.localeCompare(b.role)); break;
        case "tasks":
          users.sort((a,b)=>countTasks(b.id) - countTasks(a.id)); break;
      }
    }

    usersList.innerHTML = "";
    if (users.length === 0) {
      usersList.innerHTML = "<p class='utility-muted'>No hay usuarios registrados.</p>";
      return;
    }
    ...
    users.forEach(user => {
      ...
      card.innerHTML = `
        <div class="user-card__info">
          ...
          <div class="user-card__estado">
            Estado: <button data-js="toggle-active" class="user-card__btn">${user.activo ? "Activo" : "Inactivo"}</button>
          </div>
      ...
      card.querySelector('[data-js="toggle-active"]').addEventListener("click", () => {
        ...
        const updated = usersData.map(u=>u.id===user.id ? {...u, activo: user.activo}: u);
  ...
  function countTasks(userId){
    const tasks = JSON.parse(localStorage.getItem(TASKS_KEY)) || [];
    return tasks.filter(t=>t.asignado===userId).length;
  }
});
